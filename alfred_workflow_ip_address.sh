...
LOCAL_RESULT_HEADER=$(cat <<EOF
<?xml version="1.0"?>
<items>
EOF
)

LOCAL_RESULT_FOOTER=$(cat <<EOF
...
LOCAL_RESULT=$LOCAL_RESULT_HEADER

LOCAL_RESULT_COUNT=0
LOCAL_ARR=($(echo $LOCAL | tr "\\n" $IFS))
for value in ${LOCAL_ARR[@]};
do
    LOCAL_RESULT_TEMP=$(cat <<EOF
...
EOF
    ...
    LOCAL_RESULT=$LOCAL_RESULT$LOCAL_RESULT_TEMP
    LOCAL_RESULT_COUNT=`expr $LOCAL_RESULT_COUNT + 1`
done

LOCAL_RESULT=$LOCAL_RESULT$LOCAL_RESULT_FOOTER
